...
export function TrustCtaSection({ eyebrow, title, description, primaryAction, secondaryAction, className }: TrustCtaSectionProps) {
  return (
    <section className={cn("bg-[#191919] py-20 text-white", className)}>
      <div className="mx-auto max-w-[1310px] px-2 text-center sm:px-4 lg:px-6">
...
function CtaLink({ action, primary = false }: { action: CtaAction; primary?: boolean }) {
  const className = primary
    ? "inline-flex items-center gap-2 rounded-full bg-[#1473E6] px-8 py-3 text-lg font-semibold text-white transition-colors hover:bg-blue-700"
    : "inline-flex items-center rounded-full border border-gray-500 px-8 py-3 text-lg font-semibold text-white transition-colors hover:bg-gray-800";
